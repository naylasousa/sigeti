...
class AuthController extends Controller
{
    ...
    public function authenticate(?array $data): void
    {
        ...
        //se encontrou usuario, entao verifico o status
        ...
        //verificar se é tecnico ou professor. Se for tecnico, vai para onde? /tecnico/dashboard
        if($user->getRole() === User::TECHNICIAN){
            Message::success("Bem-vindo(a) " . $user->getName());
            redirect("/tecnico/dashboard");
            return;
        }
        // se for professor, vai pra onde? /professor/dashboard
    ...
    public function logout(?array $data): void
    {
        ...
        $session->unset("auth");
        Message::success("Sua sessão foi encerrada, mas volte logo!");
        redirect("/entrar");
    }
    ...
    public function storySuccess(): void
    {
    ...
    public function sendResetLink(?array $data): void
    {
        ...
        $user = User::findByEmail($data['email']);
        if(!$user){
            Message::success("se o e-mail estiver cadastrado, Você receberá o link de redefiniçâo de senha");
            redirect("/redefinir-senha");
            return;
        }
        ...
        try{
           ...
           Message::error("Não foi possivel enviar o e-mail. Tente novamente mais tarde.");
           redirect("/redefinir-senha");
           return;
        }
